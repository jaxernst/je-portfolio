            ...
            <div class="ml-5 pt-5 left">
                ...
                {#if started}
                    <div in:slide={{ easing: cubicInOut, duration: 250 }}>
                        ...
                        >
                            <h1 class="text-[33px] md:text-[35px]">
                                Jackson Ernst
                            </h1>
                        ...
                        <!-- Tab Buttons -->
                        <div class="flex flex-wrap gap-y-1 mt-3 max-w-[620px]">
                            {#each tabs as tab, tabIdx}
                                {@const selected = tabIdx === $tabIndex}
                                <button
                                    on:click={(e) =>
                                        handleSpawnButtonClick(e, tabIdx)}
                                    class="rounded-lg transition-all tracking-tight duration-200"
                                    style={`
                                  backdrop-filter: blur(${selected ? "5px" : "0px"});
                                  background-color: ${selected ? tab.boidType.color : 0}40;
                                  border:  1.5px solid ${selected ? tab.boidType.color : "transparent"};
                                  font-weight: ${selected ? "700" : "400"};   
                                  ${$tabIndex == tabIdx ? "opacity: 1;" : "opacity: 0.7;"}
                              `}
                                >
                                    {tab.text}
                                </button>
